...
const PurchaseDialog = ({ service, open, onPurchase, onOpenChange }) => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (service) {
      ...
      if (service.is_zddk_product && qtyValues) {
        if (
          service.product_type === "amount" &&
          typeof qtyValues === "object" &&
          qtyValues.min
        ) {
          initialQuantity = parseInt(qtyValues.min);
        } else if (
          service.product_type === "specificPackage" &&
      ...
      // لا حاجة لـ JSON.parse() هنا بعد الآن، البيانات تأتي جاهزة
      const requiredParams = Array.isArray(service.zddk_required_params)
        ? service.zddk_required_params
        : [];
          ...
          <div className="bg-muted p-3 rounded-md">
            ...
            {service.is_zddk_product && parsedZddkQtyValues && (
              <div className="text-xs  mb-2">
                {typeof parsedZddkQtyValues === "object" &&
                  parsedZddkQtyValues.min &&
                  parsedZddkQtyValues.max && (
                    <span>
                      الكمية: {parsedZddkQtyValues.min} -{" "}
                      {parsedZddkQtyValues.max}
                    </span>
                  )}
                {Array.isArray(parsedZddkQtyValues) &&
                  parsedZddkQtyValues.length > 0 && (
                    <span>
                      الكميات المتاحة: {parsedZddkQtyValues.join(", ")}
                    </span>
                  )}
              </div>
            )}
          ...
          <div className="flex flex-col space-y-2">
            ...
            parsedZddkQtyValues.length > 0 ? (
              ...
              <Input
                ...
                min={
                  service.is_zddk_product &&
                  parsedZddkQtyValues !== null &&
                  typeof parsedZddkQtyValues === "object" &&
                  parsedZddkQtyValues.min
                    ? parseInt(parsedZddkQtyValues.min)
                    : 1
                }
                max={
                  service.is_zddk_product &&
                  ...
                  typeof parsedZddkQtyValues === "object" &&
                  parsedZddkQtyValues.max
                    ? parseInt(parsedZddkQtyValues.max)
                    : undefined
                }
                value={quantity}
              ...
              {parsedZddkRequiredParams.map((paramName) => (
                <div className="flex flex-col space-y-2 mb-3" key={paramName}>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleExtraParamChange(paramName, e.target.value)
                    }
