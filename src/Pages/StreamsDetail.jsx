...
const StreamsDetail = () => {
  ...
  const priceBasedOnRole =
    user?.role === "seals" ? stream.price_wholesale : stream.price;
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="bg-white ">
          <DialogHeader>
            ...
            <DialogDescription>
              Subscribe to <strong>{stream.name_en || stream.name_ar}</strong>{" "}
              for <strong>{duration.replace("_", " ")}</strong> at{" "}
              <strong>${total}</strong>
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="py-4 space-y-4">
            ...
            <div>
              <Label htmlFor="iptv">IPTV Link</Label>
              <input
                type="text"
                name="iptv"
                value={data.iptv}
                onChange={handleInputChange}
                placeholder="Enter IPTV link"
                className="w-full p-2 rounded bg-black border border-gray-600 mt-1 text-white"
              />
            </div>
