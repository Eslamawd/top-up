...
const ServiceDetail = () => {
  ...
  const handleBuy = async () => {
    ...
    for (const field of service.user_fields || []) {
      if (
        field.required &&
        (!extraParamsInput[field.field_name] ||
          extraParamsInput[field.field_name].trim() === "")
      ) {
    ...
    const userFieldsArray = Object.entries(extraParamsInput).map(
      ([key, value]) => ({
        field_name: key,
        value: value,
      })
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="bg-white">
          <DialogHeader>
            ...
            <DialogDescription>
              Confirm buying {service.name_en} for ${totalPrice}
            </DialogDescription>
          </DialogHeader>
              ...
              <>
                ...
                {service.user_fields.map((field) => (
                  <p key={field.id} className="text-sm">
                    ...
                    {extraParamsInput[field.field_name] || "N/A"}
                  </p>
                ))}
              </>
            )}
